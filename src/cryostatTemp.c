...
unsigned int currentCryostatTempSensorAdd[CRYOSTAT_TEMP_SENSORS_NUMBER] = {
    0x0000,   // Sensor 0 -> TVO Sensor 4K cryocooler
...
void sequentialCoeffHandler(int currentCryostatModule, int currentCryostatTempModule) {
    ...
    if (++coeff >= TVO_COEFFS_NUMBER) coeff = TVO_COEFF_0;
    frontend.cryostat.cryostatTemp[currentCryostatModule].nextCoeff = coeff;
}

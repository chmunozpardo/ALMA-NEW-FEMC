...
void sisHeaterHandler(void) {
    ...
    if (frontend.cartridge[currentModule].polarization[currentBiasModule].sisHeater.available == UNAVAILABLE) {
...
static void enableHandler(void) {
    ...
    if (CAN_SIZE) {
        ...
        SAVE_LAST_CONTROL_MESSAGE(
            frontend.cartridge[currentModule].polarization[currentBiasModule].sisHeater.lastEnable)

        // If we are in STANDBY2 mode, return HARDW_BLKD_ERR
        if (frontend.cartridge[currentModule].standby2) {
            /* Store the ERROR state in the last control message variable */
            frontend.cartridge[currentModule].polarization[currentBiasModule].sisHeater.lastEnable.status =
                HARDW_BLKD_ERR;
        ...
        if ((currentModule == BAND9) && CAN_BYTE) {
            switch (queryAsyncTimer(TIMER_BIAS_B9_HEATER(currentBiasModule))) {
                case TIMER_NOT_RUNNING:
                    /* Start timer */
                    startAsyncTimer(TIMER_BIAS_B9_HEATER(currentBiasModule), TIMER_BIAS_TO_B9_HEATER, FALSE);
                    break;
                case TIMER_EXPIRED:
                    /* Reload timer */
                    startAsyncTimer(TIMER_BIAS_B9_HEATER(currentBiasModule), TIMER_BIAS_TO_B9_HEATER, TRUE);
                    break;
                case TIMER_RUNNING:
...
static void currentHandler(void) {
    ...
    if (getSisHeater() == ERROR) {
        ...
        CONV_FLOAT = frontend.cartridge[currentModule].polarization[currentBiasModule].sisHeater.current;
    } else {

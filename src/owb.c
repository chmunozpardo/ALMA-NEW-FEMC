...
/* Initialize the device discovery algorithm */
    ...
    for (device = 0; device < MAX_DEVICES_NUMBER; device++) {
#ifdef DEBUG_OWB
...
/* Send the bus reset and check for presence pulse. */
        ...
        if (startAsyncTimer(TIMER_OWB_RESET, TIMER_TO_OWB_RESET, FALSE) == ERROR) {
            return ERROR;
        }
...
/* Transmit first set of search data */
        ...
        for (loop = 0; loop < SEARCH_BYTES_LENGTH; loop++) {
...
/* Check state of search tree */
        ...
        if (RecoverROM(RData, TData, ESNS[device]) == TRUE) {
#ifdef DEBUG_OWB
            printf("done!\n");       // Search tree state
    ...
    /* Print devices list */
    for (device = 0; device < esnDevicesFound; device++) {
        printf("    - ESN%d: %02X %02X %02X %02X %02X %02X %02X %02X\n", device, ESNS[device][0], ESNS[device][1],
               ESNS[device][2], ESNS[device][3], ESNS[device][4], ESNS[device][5], ESNS[device][6], ESNS[device][7]);
    }
#ifdef DEBUG_OWB
    printf("done!\n\n");
#endif
...
int owbReset(void) {
    /* Send bus reset */
    outp(MUX_OWB_COMMAND, OWB_RESET);
...
int waitIrq(unsigned char irq) {
    ...
    if (startAsyncTimer(TIMER_OWB_IRQ, TIMER_TO_OWB_IRQ, FALSE) == ERROR) {
        return ERROR;
    }
...
int RecoverROM(int* ReceiveData, int* TransmitData, unsigned char* ROMCode) {
    ...
    for (loop = 0; loop < 16; loop++) {
        if ((ReceiveData[loop] & 0x02) == 0x00)
            RROM[loop * 4] = 0;
        else
            RROM[loop * 4] = 1;
        if ((ReceiveData[loop] & 0x08) == 0x00)
            RROM[loop * 4 + 1] = 0;
        else
            RROM[loop * 4 + 1] = 1;
        if ((ReceiveData[loop] & 0x20) == 0x00)
            RROM[loop * 4 + 2] = 0;
        else
            RROM[loop * 4 + 2] = 1;
        if ((ReceiveData[loop] & 0x80) == 0x00)
            RROM[loop * 4 + 3] = 0;
        else
            RROM[loop * 4 + 3] = 1;

        if ((ReceiveData[loop] & 0x01) == 0x00)
            RDIS[loop * 4] = 0;
        else
            RDIS[loop * 4] = 1;
        if ((ReceiveData[loop] & 0x04) == 0x00)
            RDIS[loop * 4 + 1] = 0;
        else
            RDIS[loop * 4 + 1] = 1;
        if ((ReceiveData[loop] & 0x10) == 0x00)
            RDIS[loop * 4 + 2] = 0;
        else
            RDIS[loop * 4 + 2] = 1;
        if ((ReceiveData[loop] & 0x40) == 0x00)
            RDIS[loop * 4 + 3] = 0;
        else
            RDIS[loop * 4 + 3] = 1;
    }
    ...
    for (loop = 0; loop < 16; loop++) {
        TransmitData[loop] =
            (TROM[loop * 4] << 1) + (TROM[loop * 4 + 1] << 3) + (TROM[loop * 4 + 2] << 5) + (TROM[loop * 4 + 3] << 7);
    }

    // Convert the individual recovered ROM bits into an 8 byte format

    for (loop = 0; loop < 8; loop++) {
        ROMCode[loop] = (RROM[loop * 8]) + (RROM[loop * 8 + 1] << 1) + (RROM[loop * 8 + 2] << 2) +
                        (RROM[loop * 8 + 3] << 3) + (RROM[loop * 8 + 4] << 4) + (RROM[loop * 8 + 5] << 5) +
                        (RROM[loop * 8 + 6] << 6) + (RROM[loop * 8 + 7] << 7);
    }
    if (result == -1)
        return 1;  // There are no DIS bits that haven't been flipped

...
#define MACRO_TVO_SENSOR_NAMES                                                                                       \
    {                                                                                                                \
        "CRYOCOOLER_4K", "PLATE_4K_LINK_1", "PLATE_4K_LINK_2", "PLATE_4K_FAR_1", "PLATE_4K_FAR_2", "CRYOCOOLER_12K", \
            "PLATE_12K_LINK", "PLATE_12K_FAR", "PLATE_12K_SHIELD"                                                    \
    }
...
static HANDLER cryostatModulesHandler[CRYOSTAT_MODULES_NUMBER] = {
    ...
    gateValveHandler,    solenoidValveHandler, vacuumControllerHandler, supplyCurrent230VHandler, coldHeadHoursHandler};
...
/* Cryostat availability */
    ...
    if (ReadCfg(FRONTEND_CONF_FILE, CRYO_CONF_FILE_SECTION, &dataIn) != CRYO_AVAIL_EXPECTED) {
    ...
    if (myReadCfg(frontend.cryostat.coldHeadHoursFile, CRYO_HOURS_FILE_SECTION, &dataIn, CRYO_HOURS_FILE_EXPECTED) !=
        NO_ERROR) {
    ...
    for (sensor = 0; sensor < TVO_SENSORS_NUMBER; sensor++) {
        ...
        if (myReadCfg(frontend.cryostat.configFile, TVO_NO_SECTION(sensor), &dataIn, TVO_NO_EXPECTED) != NO_ERROR) {
            return NO_ERROR;
        }
...
/* Print sensor information */
        ...
        /* Access configuration file, if error, skip the configuration. */
        if (myReadCfg(frontend.cryostat.configFile, TVO_COEFFS_SECTION(sensor), &dataIn, TVO_COEFFS_EXPECTED) !=
            NO_ERROR) {
            return NO_ERROR;
        }
...
/* Set the default value for the temperature and pressure sensors to
   FLOAT_UNINIT to allow verification that they've been monitored. */
    ...
    printf("    - Pressure...\n");
#endif
    for (sensor = 0; sensor < VACUUM_SENSORS_NUMBER; sensor++) {
        frontend.cryostat.vacuumController.vacuumSensor[sensor].pressure = FLOAT_UNINIT;
    }

#ifdef DEBUG_STARTUP
    printf("    - Temperature...\n");
#endif
    for (sensor = 0; sensor < CRYOSTAT_TEMP_SENSORS_NUMBER; sensor++) {
...
int cryostatSensorTablesReport(void) {
    ...
    for (sensor = 0; sensor < TVO_SENSORS_NUMBER; sensor++) {
        ...
        for (coeff = 0; coeff < TVO_COEFFS_NUMBER; coeff++) {
            ...
            if (coeff < TVO_COEFFS_NUMBER - 1) printf(",");
        }
        printf("\n\n");
    }
    return NO_ERROR;
...
int cryostatAsyncLogHours(void) {
    ...
    switch (asyncCryoLogHoursState) {
        case ASYNC_CRYO_LOG_HOURS_SET_TIMER:
            ...
            if (startAsyncTimer(TIMER_CRYO_LOG_HOURS, TIMER_CRYO_LOG_HOURS_WAIT, FALSE) == ERROR) {

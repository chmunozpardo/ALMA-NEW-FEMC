...
void turboPumpEnableHandler(void) {
    ...
    if (CAN_SIZE) {
        ...
        SAVE_LAST_CONTROL_MESSAGE(frontend.cryostat.turboPump.lastEnable)
        ...
        if (frontend.cryostat.backingPump.enable == BACKING_PUMP_DISABLE) {
            frontend.cryostat.turboPump.lastEnable.status =
                HARDW_BLKD_ERR;  // Store the status in the last control message
        ...
        if (CAN_BYTE && frontend.fetim.available == AVAILABLE) {
            if ((frontend.fetim.compressor.temp[FETIM_EXT_SENSOR_TURBO].temp < TURBO_PUMP_MIN_TEMPERATURE) ||
                (frontend.fetim.compressor.temp[FETIM_EXT_SENSOR_TURBO].temp > TURBO_PUMP_MAX_TEMPERATURE)) {
                storeError(ERR_TURBO_PUMP,
                           ERC_HARDWARE_BLOCKED);  // Temperature below allowed range -> Turbo pump disabled
                frontend.cryostat.turboPump.lastEnable.status =
                    HARDW_BLKD_ERR;  // Store the status in the last control message

                frontend.cryostat.turboPump.enable = TURBO_PUMP_DISABLE;
                return;
            }
        }
        ...
        /* Change the status of the turbo pump according to the content of the
           CAN message. */
        if (setTurboPumpEnable(CAN_BYTE ? TURBO_PUMP_ENABLE : TURBO_PUMP_DISABLE) == ERROR) {
            /* Store the ERROR state in the last control message variable */
            frontend.cryostat.turboPump.lastEnable.status = ERROR;
    ...
    if (currentClass == CONTROL_CLASS) {
        ...
        RETURN_LAST_CONTROL_MESSAGE(frontend.cryostat.turboPump.lastEnable)
        return;
    }
    ...
    CAN_BYTE = frontend.cryostat.turboPump.enable;
...
void turboPumpStateHandler(void) {
    ...
    prevErrorState = frontend.cryostat.turboPump.state = cryoRegisters.statusReg.bitField.turboPumpError;
    ...
    if (getTurboPumpStates() == ERROR) {
        ...
        CAN_BYTE = frontend.cryostat.turboPump.state;
    } else {
    ...
    if (prevErrorState != frontend.cryostat.turboPump.state) {
        if (frontend.cryostat.turboPump.state == 1) {
...
void turboPumpSpeedHandler(void) {
    ...
    if (getTurboPumpStates() == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CAN_BYTE = frontend.cryostat.turboPump.speed;
    } else {
        /* If no error during monitor process, gather the stored data */
        CAN_BYTE = frontend.cryostat.turboPump.speed;
    }

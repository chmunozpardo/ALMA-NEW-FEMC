...
static void lockDetectVoltageHandler(void) {
    ...
    if (getPll(PLL_LOCK_DETECT_VOLTAGE) == ERROR) {
        ...
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.lockDetectVoltage;
    } else {
...
static void correctionVoltageHandler(void) {
    ...
    if (getPll(PLL_CORRECTION_VOLTAGE) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.correctionVoltage;
    } else {
        /* If no error during monitor process, gather the stored data */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.correctionVoltage;
    }
...
static void assemblyTempHandler(void) {
    ...
    if (getPll(PLL_ASSEMBLY_TEMP) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.assemblyTemp;
    } else {
        /* If no error during monitor process, gather the stored data */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.assemblyTemp;
    }
...
static void YIGHeaterCurrentHandler(void) {
    ...
    if (getPll(PLL_YIG_HEATER_CURRENT) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.YIGHeaterCurrent;
    } else {
        /* If no error during monitor process, gather the stored data */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.YIGHeaterCurrent;
    }
...
static void refTotalPowerHandler(void) {
    ...
    if (getPll(PLL_REF_TOTAL_POWER) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.refTotalPower;
    } else {
        /* If no error during monitor process, gather the stored data */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.refTotalPower;
    }
...
static void ifTotalPowerHandler(void) {
    ...
    if (getPll(PLL_IF_TOTAL_POWER) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.ifTotalPower;
    } else {
        /* If no error during monitor process, gather the stored data */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.pll.ifTotalPower;
    }
...
static void unlockDetectLatchHandler(void) {
    ...
    if (getPllStates() == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CAN_BYTE = frontend.cartridge[currentModule].lo.pll.unlockDetectLatch;
    } else {
        /* If no error during monitor process, gather the stored data */
        CAN_BYTE = frontend.cartridge[currentModule].lo.pll.unlockDetectLatch;
    }
...
static void clearUnlockDetectLatchHandler(void) {
    ...
    if (CAN_SIZE) {  // If control (size !=0)
        ...
        SAVE_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastClearUnlockDetectLatch)
    ...
    if (currentClass == CONTROL_CLASS) {
        ...
        RETURN_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastClearUnlockDetectLatch)
        return;
    }
...
static void loopBandwidthSelectHandler(void) {
    ...
    if (currentClass == CONTROL_CLASS) {
        // return the last control message and status
        RETURN_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastLoopBandwidthSelect)
        return;
    }
    ...
    /* This monitor point doesn't return an hardware status but just the
       ...
       control command. */
    CAN_BYTE = frontend.cartridge[currentModule].lo.pll.loopBandwidthSelect;
    CAN_SIZE = CAN_BOOLEAN_SIZE;
}
...
static void sidebandLockPolaritySelectHandler(void) {
    ...
    if (CAN_SIZE) {  // If control (size !=0)
        // save the incoming message:
        SAVE_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastSidebandLockPolaritySelect)
    ...
    if (currentClass == CONTROL_CLASS) {
        // return the last control message and status
        RETURN_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastSidebandLockPolaritySelect)
        return;
    }
    ...
    /* This monitor point doesn't return an hardware status but just the
       ...
       update when the state of the mixer bias mode is changed by a
       control command. */
    CAN_BYTE = frontend.cartridge[currentModule].lo.pll.sidebandLockPolaritySelect;
    CAN_SIZE = CAN_BOOLEAN_SIZE;
}
...
static void nullLoopIntegratorHandler(void) {
    ...
    if (CAN_SIZE) {  // If control (size !=0)
        // save the incoming message:
        SAVE_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastNullLoopIntegrator)
    ...
    if (currentClass == CONTROL_CLASS) {
        // return the last control message and status
        RETURN_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.pll.lastNullLoopIntegrator)
        return;
    }
    ...
    /* This monitor point doesn't return an hardware status but just the
       ...
       update when the state of the mixer bias mode is changed by a
       control command. */
    CAN_BYTE = frontend.cartridge[currentModule].lo.pll.nullLoopIntegrator;
    CAN_SIZE = CAN_BOOLEAN_SIZE;
}

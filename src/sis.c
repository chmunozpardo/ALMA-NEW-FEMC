    ...
    if (CAN_SIZE) {
        ...
        for (pol = 0; pol < POLARIZATIONS_NUMBER; pol++) {
            for (sb = 0; sb < SIDEBANDS_NUMBER; sb++) {
                ...
                frontend.cartridge[currentModule].polarization[pol].sideband[sb].sis.resistor = CONV_FLOAT;
            }
        }
    ...
    if (CAN_SIZE) {
        ...
        if (frontend.cartridge[currentModule].standby2) {
            ...
            frontend.cartridge[currentModule]
                .polarization[currentBiasModule]
                .sideband[currentPolarizationModule]
                .sis.lastVoltage.status = HARDW_BLKD_ERR;
            return;
        }
...
void openLoopHandler(int currentModule, int currentBiasModule, int currentPolarizationModule) {
    ...
    if (CAN_SIZE) {
        ...
        if (frontend.cartridge[currentModule].standby2) {
            ...
            frontend.cartridge[currentModule]
                .polarization[currentBiasModule]
                .sideband[currentPolarizationModule]
                .sis.lastOpenLoop.status = HARDW_BLKD_ERR;
            return;
        }
    ...
    /* This monitor point doesn't return an hardware status but just the
       ...
       control command. */
    CAN_BYTE = frontend.cartridge[currentModule]
                   .polarization[currentBiasModule]
                   .sideband[currentPolarizationModule]
                   .sis.openLoop;

...
void pmxEnableHandler(int currentModule) {
    ...
    if (CAN_SIZE) {  // If control (size !=0)
        ...
        SAVE_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.photomixer.lastEnable)
    ...
    if (currentClass == CONTROL_CLASS) {
        ...
        RETURN_LAST_CONTROL_MESSAGE(frontend.cartridge[currentModule].lo.photomixer.lastEnable)
        return;
    }
    ...
    /* This monitor point doesn't return an hardware status but just the
       ...
       control command. */
    CAN_BYTE = frontend.cartridge[currentModule].lo.photomixer.enable;
...
void pmxVoltageHandler(int currentModule) {
    ...
    if (getPhotomixer(PHOTOMIXER_BIAS_V, currentModule) == ERROR) {
        ...
        CONV_FLOAT = frontend.cartridge[currentModule].lo.photomixer.voltage;
    } else {
...
void pmxCurrentHandler(int currentModule) {
    ...
    if (getPhotomixer(PHOTOMIXER_BIAS_C, currentModule) == ERROR) {
        ...
        CAN_STATUS = ERROR;
        /* Store the last known value in the outgoing message */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.photomixer.current;
    } else {
        /* If no error during the monitor process, gather the stored data. */
        CONV_FLOAT = frontend.cartridge[currentModule].lo.photomixer.current;
    }

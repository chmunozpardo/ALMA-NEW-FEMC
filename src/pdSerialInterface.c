...
/* Initiate ADC conversion:
   - send ADC convert strobe command */
    ...
    if (startAsyncTimer(TIMER_PD_ADC_RDY, TIMER_PD_TO_ADC_RDY, FALSE) == ERROR) {
        return ERROR;
    }
    ...
    } while ((pdRegisters.statusReg.bitField.adcReady == PD_ADC_BUSY) && (timedOut == TIMER_RUNNING));
...
int setPdModuleEnable(unsigned char enable) {
    ...
    int tempAReg = pdRegisters.aReg;

    if (frontend.mode != SIMULATION_MODE) {
        /* Update AREG */
        pdRegisters.aReg = (enable == PD_MODULE_ENABLE) ? SET_PD_MODULE_ENABLE(currentPowerDistributionModule)
...
/* 2 - Parallel write AREG */
    ...
    frontend.powerDistribution.pdModule[currentPowerDistributionModule].enable =
        (enable == PD_MODULE_ENABLE) ? PD_MODULE_ENABLE : PD_MODULE_DISABLE;
...
int getPdChannel(void) {
    ...
    if (frontend.mode != SIMULATION_MODE) {
        ...
        pdRegisters.bReg.integer = 0x0000;

        /* 1 - Select the desired monitor point
               a - update BREG */
        pdRegisters.bReg.bitField.monitorBand = PD_BREG_SELECT_BAND(currentPowerDistributionModule);
        pdRegisters.bReg.bitField.monitorPoint = PD_BREG_SELECT_MONITOR(currentPdModuleModule, currentPdChannelModule);
        ...
        switch (currentPdChannelModule) {
            case PD_CHANNEL_CURRENT:
                ...
                frontend.powerDistribution.pdModule[currentPowerDistributionModule]
                    ...
                    .current = (scale * pdRegisters.adcData) / PD_ADC_RANGE;
                break;
            case PD_CHANNEL_VOLTAGE:
                ...
                frontend.powerDistribution.pdModule[currentPowerDistributionModule]
                    .pdChannel[currentPdModuleModule]
                    .voltage = (scale * pdRegisters.adcData) / PD_ADC_RANGE;
                break;
            default:
                break;
        }
        ...
        switch (currentPdChannelModule) {
            case PD_CHANNEL_CURRENT:
                frontend.powerDistribution.pdModule[currentPowerDistributionModule]
                    .pdChannel[currentPdModuleModule]
                    .current = (float)frontend.powerDistribution.pdModule[currentPowerDistributionModule].enable * 1.0 +
                               (float)currentPdModuleModule * 0.01;
                break;
            case PD_CHANNEL_VOLTAGE:
                switch (currentPdModuleModule) {
                    case PLUS_6:

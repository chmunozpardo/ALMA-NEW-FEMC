...
static unsigned long
    asyncMSeconds[MAX_TIMERS_NUMBER];  // A global for the async timer wait time
...
int startAsyncTimer(unsigned char timerNo, unsigned long mSeconds,
    ...
    if (timerNo > MAX_TIMERS_NUMBER) {
        storeError(ERR_TIMER,
                   ERC_MODULE_RANGE);  // Required async timer out of range
        return ERROR;
    }
    ...
    if (asyncRunning[timerNo]) {
        if (reload == FALSE) {
            storeError(ERR_TIMER,
                       ERC_HARDWARE_WAIT);  // Async timer already running
            return ERROR;
        }
    }

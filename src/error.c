...
void criticalError(unsigned char moduleNo, unsigned char errorNo) {
...
void reportErrorConsole(unsigned char moduleNo, unsigned char errorNo) {
    ...
    switch (errorNo) {
        ...
        case ERC_HARDWARE_ERROR:  // Hardware is in an error state
            sprintf(error, "Module is in an error state");
            break;

        case ERC_HARDWARE_WAIT:  // Waiting for previous command to finish
            sprintf(error, "Waiting for previous command to finish");
            break;

        case ERC_FLASH_ERROR:  // Error reading/writing flash disk
            sprintf(error, "Error accessing flash disk");
            break;

        case ERC_HARDWARE_BLOCKED:  //!< Command blocked by safety check
            sprintf(error, "Command blocked by safety check");
            break;

        case ERC_DEBUG_ME:  // Software entered an invalid or impossible state
            sprintf(error, "Software entered an invalid or impossible state");
            break;
        ...
        case ERC_COMMAND_VAL:  // Command value out of range
            sprintf(error, "Error: The command vaue is out of range");
            break;

        case ERC_FPGA_NOT_READY:
            sprintf(error, "Critical Error: Serial Mux - FPGA not ready");
            break;

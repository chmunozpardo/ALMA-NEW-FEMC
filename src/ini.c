...
static char *chgext(char *path, char *oldext, char *newext) {
    ...
    if (oldext && strcmp(p, oldext)) return NULL;
...
static char *StripLeadingSpaces(char *string) {
    if (!string || (0 == strlen(string))) return NULL;
...
static char *StripTrailingSpaces(char *string) {
    ...
    while (isspace(LAST_CHAR(string))) LAST_CHAR(string) = NUL;
...
static enum LineTypes SectionLine(char *line, char *SectionWanted, char *CurrentSection) {
    ...
    line = StripLeadingSpaces(line);
    if (!line || NUL == *line) return EmptyLine;

    /*
    ** Comments are started with a "%", ";" or "#"
    */

    if (';' == *line) return CommentLine;
    if ('%' == *line) return CommentLine;
    if ('#' == *line) return CommentLine;
    ...
    {
        linetype = NewSection;
        if (StrEq(CurrentSection, SectionWanted)) linetype = LeavingSection;

        strcpy(CurrentSection, line);
        if (StrEq(line, SectionWanted)) linetype = FoundSection;
    } else { /* Just a regular line */
        linetype = NotInSection;
        if (StrEq(CurrentSection, SectionWanted)) linetype = InSection;
    }
...
int ReadCfg(const char *FileName, char *SectionName, CFG_STRUCT *MyVars) {
    FILE *CfgFile;
...
int UpdateCfg(const char *FileName, char *SectionName, char *VarWanted, char *NewData) {
    ...
    */
    ...
    if (ferror(CfgFile)) Error = -1;
    if (ferror(NewCfgFile)) Error = -1;
    fclose(CfgFile);
    fclose(NewCfgFile);

    if (!Error) {
        if (remove(FileName)) return -1;
        if (rename(TempFileName, FileName)) return -1;
    }
    return Error;
}

...
#define CAN_RX_MESSAGE_SIZE \
    0x0D  // Maximum size of the can message transmitted by the AMBSI1 on the
          // parallel port
#define CAN_RX_MAX_PAYLOAD_SIZE \
...
#define CLASSES_RCA_MASK                                                 \
    0x30000L                   /* Mask to extract the different classes: \
                                  0 -> Monitor Messages                  \
                                  1 -> Control Messages                  \
                                  2 -> Special Messages                  \
...
#define MODULES_RCA_MASK                                                     \
    0x0F000                    /* Mask to extract the module number:         \
...
#define GET_AMBSI1_VERSION_INFO \
    0x20000L  //!< \b BASE+0x00 -> AMBSI1 dedicated message to get firmware
              //!< version. It should never be received from this software
#define GET_SETUP_INFO \
    0x20001L  //!< \b BASE+0x01 -> AMBSI1 dedicated message to get setup info.
              //!< It should never be received from this software
#define GET_ARCOM_VERSION_INFO \
    0x20002L  //!< \b BASE+0x02 -> Information about the ARCOM Pegasus firware
              //!< version. This is the first addressable special RCA (Don't
              //!< change the offset of this RCA, since it is register with the
              //!< same number in the AMBSI1)
#define GET_SPECIAL_MONITOR_RCAS \
    0x20003L  //!< \b BASE+0x03 -> Information about the special monitor RCA
              //!< range (Don't change the offset of this RCA, since it is
              //!< register with the same number in the AMBSI1)
#define GET_SPECIAL_CONTROL_RCAS \
    0x20004L  //!< \b BASE+0x04 -> Information about the special control RCA
              //!< range (Don't change the offset of this RCA, since it is
              //!< register with the same number in the AMBSI1)
#define GET_MONITOR_RCAS \
    0x20005L  //!< \b BASE+0x05 -> Information about the monitor RCA range
              //!< (Don't change the offset of this RCA, since it is register
              //!< with the same number in the AMBSI1)
#define GET_CONTROL_RCAS \
    0x20006L  //!< \b BASE+0x06 -> Information about the control RCA range
              //!< (Don't change the offset of this RCA, since it is register
              //!< with the same number in the AMBSI1)
...
#define SET_LO_CLEAR_PA_LIMITS \
    0x21020L  //!< \b BASE+0x20 through 0x29 clear the PA LIMITS table for band
              //!< 1-10
    ...
    /*! This contains the error state of the current CAN operation. This
        ...
        standard CAN monitor messages:
            - general
                ...
                - \ref ERROR            -> the operation was \em not succesful
       (error while communicating with hardware)
                - \ref HARDW_RNG_ERR    -> the addressed hardware is not
       installed
                - \ref HARDW_BLKD_ERR   -> the addressed hardware is locked
            - if monitor message on a monitor point:
                - \ref MON_CAN_RNG      -> the monitor message RCA is out of
       allowed range
            - if monitor message on a control point:
                - \ref CON_ERROR_RNG    -> the set value in the last control
       operation is outside the allowed range */
...
typedef struct {
    ...
    /*! This contains an code resulting from the last control operation:
            - \ref NO_ERROR         -> the operation was succesful and the
       control data was within the allowed range
            - \ref ERROR            -> the operation was nor succesful (error
       while communicating with the hardware)
            - \ref CON_ERROR_RNG    -> the operation was succesful but the
       control data was outside the allowed range
            - \ref HARDW_BLKD_ERR   -> the addressed hardware is locked */
    unsigned char status;
} LAST_CONTROL_MESSAGE;

//! A macro to save the incoming control message into a LAST_CONTROL_MESSAGE
//! struct

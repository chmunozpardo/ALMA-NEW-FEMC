...
#define CRYOSTAT_MODULES_RCA_MASK                                               \
    0x000FC                            /* Mask to extract the submodule number: \
                                          0-12 -> cryostatTemp                  \
                                          13   -> backingPump                   \
                                          14   -> turboPump                     \
                                          15   -> gateValve                     \
...
#define CRYOSTAT_TEMP_SANITY_CHECK(T) \
    ((T != FLOAT_ERROR) && (T != FLOAT_UNINIT) && (CRYOSTAT_TEMP_RANGE_LOW < T) && (T < CRYOSTAT_TEMP_RANGE_HIGH))
//!< sanity check macro for testing temperature values

#define CRYOSTAT_TEMP_BELOW_MAX(T, MAX) (CRYOSTAT_TEMP_SANITY_CHECK(T) && (T < MAX))
//!< below threshold and sanity check macro for temps.

#define CRYOSTAT_LOG_HOURS_THRESHOLD \
    265.0  // if any cryocooler stage is below this value,
